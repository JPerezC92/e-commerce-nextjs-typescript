...
const useBasket: TBasketHook = () => {
  ...
  const handleAdd: handleAdd = async (basketProduct) => {
    ...
    const basketProductExists = basketProducts.some(({ _id }) => _id === basketProduct._id);
    ...
    if (!basketProductExists) {
      ...
      newBasketProducstState = [
        ...basketProducts.map((bp) =>
          bp._id === basketProduct._id ? { ...bp, quantity: bp.quantity + 1 } : bp
        ),
      ];
    }

    const responseData = await BasketService.updateBasket(basketId, newBasketProducstState);

    if (!responseData.error) setBasketProducts(newBasketProducstState);
  };

  const handleDelete: handleDelete = async (basketProductId) => {
    const newBasketProducstState = basketProducts.filter(({ _id }) => _id !== basketProductId);
    const responseData = await BasketService.updateBasket(basketId, newBasketProducstState);
    if (!responseData.error) setBasketProducts(newBasketProducstState);
  };
